...
async function testConnection(conn) {
  const pool = new Pool({
    ...
    database: conn.defaultDatabase || 'postgres', max: 1, connectionTimeoutMillis: 6000,
  });
  try {
...
async function runQuery(conn, database, sql, maxRows) {
  ...
  if (result.fields && result.fields.length > 0) {
    ...
    return { columns, rows, rowCount: result.rowCount ?? result.rows.length, truncated: result.rows.length > maxRows, ms };
  }
  return {
